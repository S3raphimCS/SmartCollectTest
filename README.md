...
Реализован на **Django** + **DRF** с использованием **Celery** для фоновой обработки задач.
...
### 2. Запуск проекта
Сборка и запуск контейнеров (Web, Database, Redis, Celery Worker, Celery Beat):
```bash
make build
```

### 3. Добавьте доменное имя в Nginx PM (localhost:81)
...
3. Безопасность:
   - Секретные данные (ключи, пароли) передаются через переменные окружения (Vault или Kubernetes Secrets).
